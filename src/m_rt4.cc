                ...
                const Verbosity& verbosity )
{
  ...
  run_rt4( ws, doit_i_field, scat_za_grid,
           f_grid, p_grid, z_field, t_field, vmr_field, pnd_field,
           scat_data,
...
/* Workspace method: Doxygen documentation will be auto-generated */
void RT4CalcSpectral( Workspace& ws,
                // WS Output:
                Tensor7& doit_i_field,
                Vector& scat_za_grid,
                Vector& scat_aa_grid,
                ...
                const Verbosity& verbosity )
{
  ...
  const String quad_type = quadtype.toupper();
  Index nhza, nhstreams, nummu;
  check_rt4_inputSpectral( nhstreams, nhza, nummu,
                   cloudbox_on,
                   atmfields_checked, atmgeom_checked,
                   cloudbox_checked, scat_data_checked,
                   cloudbox_limits, scat_data_spectral, atmosphere_dim, stokes_dim,
                   nstreams, quad_type, add_straight_angles,
                   pnd_field.ncols() );
  init_ifield( doit_i_field, f_grid, cloudbox_limits, 2*nummu, stokes_dim );
  ...
  run_rt4_spectral( ws, doit_i_field, scat_za_grid,
           f_grid, p_grid, z_field, t_field, vmr_field, pnd_field,
           scat_data_spectral,
           propmat_clearsky_agenda,
           cloudbox_limits, stokes_dim, nummu, nhza,
...
void RT4CalcSpectral( Workspace&,
                // WS Output:
                Tensor7&,
                Vector&,
                Vector&,
                ...
                const Tensor4&,
                const Vector&,
                const ArrayOfArrayOfSpectralSingleScatteringData&,
                const Vector&,
                const Index&,
                const Index&,
                const String&,
