...
void scat_data_spectralCheck(  //Input:
  ...
  for (Index i_ss = 0; i_ss < N_ss; i_ss++) {
    ...
    // Loop over the included scattering elements
    for (Index i_se = 0; i_se < N_se; i_se++) {
      for (Index f = 0; f < scat_data_spectral[i_ss][i_se].f_grid.nelem();
           f++) {
        for (Index inc = 0;
             inc < scat_data_spectral[i_ss][i_se].abs_vec_data_real.nrows();
             inc++) {
          for (Index t = 0; t < scat_data_spectral[i_ss][i_se].T_grid.nelem();
               t++) {
            for (Index st = 0;
                 st < scat_data_spectral[i_ss][i_se].abs_vec_data_real.ncols();
                 st++)
              if (std::isnan(scat_data_spectral[i_ss][i_se].abs_vec_data_real(
                      f, t, inc, st))) {
                ostringstream os;
                os << "Scatt. species #" << i_ss << " element #" << i_se
                   << " contains NaN in abs_vec at f#" << f << ", T#" << t
                   << ", coeff_inc#" << inc << ", stokes #" << st << "\n";
                throw runtime_error(os.str());
              }
            for (Index st = 0;
                 st < scat_data_spectral[i_ss][i_se].ext_mat_data_real.ncols();
                 st++)
              if (std::isnan(scat_data_spectral[i_ss][i_se].ext_mat_data_real(
                      f, t, inc, st))) {
                ostringstream os;
                os << "Scatt. species #" << i_ss << " element #" << i_se
                   << " contains NaN in ext_mat at f#" << f << ", T#" << t
          ...
          Index nTpha =
              scat_data_spectral[i_ss][i_se].pha_mat_data_real.nbooks();
          for (Index t = 0; t < nTpha; t++) {
            for (Index sca = 0;
                 sca <
              ...
              for (Index st = 0;
                   st <
                   scat_data_spectral[i_ss][i_se].pha_mat_data_real.ncols();
                   st++)
                if (std::isnan(scat_data_spectral[i_ss][i_se].pha_mat_data_real(
                        f, t, sca, inc, st))) {
                  ostringstream os;
                  os << "Scatt. species #" << i_ss << " element #" << i_se
                     << " contains NaN in pha_mat at f#" << f << ", T#" << t
                     << " (of " << nTpha << "), coeff_sca#" << sca
  ...
  if (check_type.toupper() == "ALL") {
    ...
    out2 << " checking normalization of scattering matrix\n";
    out1
        << "Currently, the spectral scattering data is not checked for normalisation...\n";
  } else if (check_type.toupper() == "SANE") {
    out1 << "  WARNING:\n"
         << "  Normalization check on pha_mat switched off.\n"
         << "  Scattering solution might be wrong.\n";
...
/* Workspace method: Doxygen documentation will be auto-generated */
void scat_data_spectralCalc(
    ArrayOfArrayOfSpectralSingleScatteringData& scat_data_spectral,
...
// the scat solvers needs to be reviewed again and adapted to this!
{
    ...
    for (Index i_se = 0; i_se < scat_data_spectral_raw[i_ss].nelem(); i_se++) {
      ...
      if (scat_data_spectral_raw[i_ss][i_se].f_grid.nelem() == 1 && nf == 1)
        if (!is_same_within_epsilon(
                scat_data_spectral_raw[i_ss][i_se].f_grid[0],
                f_grid[0],
                2 * DBL_EPSILON)) {
          ostringstream os;
          os << "There is a problem with the grids for the following "
      ...
      // check with extrapolation
      chk_interpolation_grids(which_interpolation,
                              scat_data_spectral_raw[i_ss][i_se].f_grid,
                              f_grid,
                              interp_order);
    }
  }
  ...
  for (Index i_ss = 0; i_ss < scat_data_spectral_raw.nelem(); i_ss++) {
    ...
    for (Index i_se = 0; i_se < N_se; i_se++) {
      //Stuff that doesn't need interpolating
      scat_data_spectral[i_ss][i_se].ptype =
          scat_data_spectral_raw[i_ss][i_se].ptype;
      scat_data_spectral[i_ss][i_se].f_grid = f_grid;
      scat_data_spectral[i_ss][i_se].T_grid =
          scat_data_spectral_raw[i_ss][i_se].T_grid;
      scat_data_spectral[i_ss][i_se].coeff_inc =
          scat_data_spectral_raw[i_ss][i_se].coeff_inc;
      scat_data_spectral[i_ss][i_se].coeff_sca =
          scat_data_spectral_raw[i_ss][i_se].coeff_sca;

      //Sizing SSD data containers
      scat_data_spectral[i_ss][i_se].pha_mat_data_real.resize(
          nf,
      ...
      scat_data_spectral[i_ss][i_se].ext_mat_data_real.resize(
          nf,
      ...
      scat_data_spectral[i_ss][i_se].abs_vec_data_real.resize(
          nf,
      ...
      scat_data_spectral[i_ss][i_se].forward_peak_data_real.resize(
          nf,
      ...
      scat_data_spectral[i_ss][i_se].backward_peak_data_real.resize(
          nf,
      ...
      scat_data_spectral[i_ss][i_se].pha_mat_data_imag.resize(
          nf,
      ...
      scat_data_spectral[i_ss][i_se].ext_mat_data_imag.resize(
          nf,
      ...
      scat_data_spectral[i_ss][i_se].abs_vec_data_imag.resize(
          nf,
      ...
      scat_data_spectral[i_ss][i_se].forward_peak_data_imag.resize(
          nf,
      ...
      scat_data_spectral[i_ss][i_se].backward_peak_data_imag.resize(
          nf,
      ...
      const bool single_se_fgrid =
          (scat_data_spectral_raw[i_ss][i_se].f_grid.nelem() == 1);
      if (!single_se_fgrid) {
        // Gridpositions:
        ArrayOfGridPosPoly freq_gp(nf);
        ;
        gridpos_poly(freq_gp,
                     scat_data_spectral_raw[i_ss][i_se].f_grid,
                     f_grid,
                     interp_order);
        ...
        for (Index t_index = 0;
             ...
             scat_data_spectral_raw[i_ss][i_se].pha_mat_data_real.nbooks();
             t_index++) {
          for (Index i_coeff_sca = 0;
               i_coeff_sca <
               scat_data_spectral_raw[i_ss][i_se].pha_mat_data_real.npages();
               i_coeff_sca++) {
            for (Index i_coeff_inc = 0;
                 i_coeff_inc <
                 scat_data_spectral_raw[i_ss][i_se].pha_mat_data_real.nrows();
                 i_coeff_inc++) {
              for (Index i = 0;
                   ...
                   scat_data_spectral_raw[i_ss][i_se].pha_mat_data_real.ncols();
                   i++) {
                interp(scat_data_spectral[i_ss][i_se].pha_mat_data_real(
                           joker, t_index, i_coeff_sca, i_coeff_inc, i),
                       itw,
                       scat_data_spectral_raw[i_ss][i_se].pha_mat_data_real(
                           joker, t_index, i_coeff_sca, i_coeff_inc, i),
                       freq_gp);
              }
            }
          ...
          for (Index t_index = 0;
            ...
            for (Index i_coeff_sca = 0;
              ...
              for (Index i_coeff_inc = 0;
                   ...
                   scat_data_spectral_raw[i_ss][i_se].pha_mat_data_imag.nrows();
                   i_coeff_inc++) {
                for (Index i = 0; i < scat_data_spectral_raw[i_ss][i_se]
                                          .pha_mat_data_imag.ncols();
                     i++) {
                  interp(scat_data_spectral[i_ss][i_se].pha_mat_data_imag(
                             joker, t_index, i_coeff_sca, i_coeff_inc, i),
                         itw,
                         scat_data_spectral_raw[i_ss][i_se].pha_mat_data_imag(
                             joker, t_index, i_coeff_sca, i_coeff_inc, i),
                         freq_gp);
                }
              }
        ...
        for (Index t_index = 0;
          ...
          for (Index i_coeff_inc = 0;
            ...
            for (Index i = 0;
                 ...
                 scat_data_spectral_raw[i_ss][i_se].ext_mat_data_real.ncols();
                 i++) {
              interp(scat_data_spectral[i_ss][i_se].ext_mat_data_real(
                         joker, t_index, i_coeff_inc, i),
                     itw,
                     scat_data_spectral_raw[i_ss][i_se].ext_mat_data_real(
                         joker, t_index, i_coeff_inc, i),
                     freq_gp);
            }
          }
          ...
          for (Index t_index = 0;
            ...
            for (Index i_coeff_inc = 0;
              ...
              for (Index i = 0;
                   ...
                   scat_data_spectral_raw[i_ss][i_se].ext_mat_data_imag.ncols();
                   i++) {
                interp(scat_data_spectral[i_ss][i_se].ext_mat_data_imag(
                           joker, t_index, i_coeff_inc, i),
                       itw,
                       scat_data_spectral_raw[i_ss][i_se].ext_mat_data_imag(
                           joker, t_index, i_coeff_inc, i),
                       freq_gp);
              }
            }
        ...
        for (Index t_index = 0;
          ...
          for (Index i_coeff_inc = 0;
            ...
            for (Index i = 0;
                 ...
                 scat_data_spectral_raw[i_ss][i_se].abs_vec_data_real.ncols();
                 i++) {
              interp(scat_data_spectral[i_ss][i_se].abs_vec_data_real(
                         joker, t_index, i_coeff_inc, i),
                     itw,
                     scat_data_spectral_raw[i_ss][i_se].abs_vec_data_real(
                         joker, t_index, i_coeff_inc, i),
                     freq_gp);
            }
          }
          ...
          for (Index t_index = 0;
            ...
            for (Index i_coeff_inc = 0;
              ...
              for (Index i = 0;
                   ...
                   scat_data_spectral_raw[i_ss][i_se].abs_vec_data_imag.ncols();
                   i++) {
                interp(scat_data_spectral[i_ss][i_se].abs_vec_data_imag(
                           joker, t_index, i_coeff_inc, i),
                       itw,
                       scat_data_spectral_raw[i_ss][i_se].abs_vec_data_imag(
                           joker, t_index, i_coeff_inc, i),
                       freq_gp);
              }
            }
        ...
        for (Index t_index = 0;
             ...
             t_index++) {
          for (Index i_coeff_inc = 0;
               i_coeff_inc < scat_data_spectral_raw[i_ss][i_se]
                                 .forward_peak_data_imag.nrows();
               i_coeff_inc++) {
            for (Index i = 0; i < scat_data_spectral_raw[i_ss][i_se]
                                      .forward_peak_data_imag.ncols();
                 i++) {
              interp(scat_data_spectral[i_ss][i_se].forward_peak_data_imag(
                         joker, t_index, i_coeff_inc, i),
                     itw,
                     scat_data_spectral_raw[i_ss][i_se].forward_peak_data_imag(
                         joker, t_index, i_coeff_inc, i),
                     freq_gp);
            }
          }
        ...
        if (scat_data_spectral_raw[i_ss][i_se].forward_peak_data_imag.npages() >
            1) {
          for (Index t_index = 0;
               t_index < scat_data_spectral_raw[i_ss][i_se]
                             .forward_peak_data_imag.npages();
               t_index++) {
            for (Index i_coeff_inc = 0;
                 i_coeff_inc < scat_data_spectral_raw[i_ss][i_se]
                                   .forward_peak_data_imag.nrows();
              ...
              for (Index i = 0; i < scat_data_spectral_raw[i_ss][i_se]
                   ...
                   i++) {
                interp(
                    scat_data_spectral[i_ss][i_se].forward_peak_data_imag(
                        joker, t_index, i_coeff_inc, i),
                    itw,
                    scat_data_spectral_raw[i_ss][i_se].forward_peak_data_imag(
                        joker, t_index, i_coeff_inc, i),
        ...
        for (Index t_index = 0; t_index < scat_data_spectral_raw[i_ss][i_se]
                                              .backward_peak_data_real.npages();
             t_index++) {
          for (Index i_coeff_inc = 0;
               i_coeff_inc < scat_data_spectral_raw[i_ss][i_se]
                                 .backward_peak_data_real.nrows();
               i_coeff_inc++) {
            for (Index i = 0; i < scat_data_spectral_raw[i_ss][i_se]
                                      .backward_peak_data_real.ncols();
                 i++) {
              interp(scat_data_spectral[i_ss][i_se].backward_peak_data_real(
                         joker, t_index, i_coeff_inc, i),
                     itw,
                     scat_data_spectral_raw[i_ss][i_se].backward_peak_data_real(
                         joker, t_index, i_coeff_inc, i),
                     freq_gp);
            }
          }
        ...
        if (scat_data_spectral_raw[i_ss][i_se]
                .backward_peak_data_imag.npages() > 1) {
          for (Index t_index = 0;
               t_index < scat_data_spectral_raw[i_ss][i_se]
                             .backward_peak_data_imag.npages();
               t_index++) {
            for (Index i_coeff_inc = 0;
                 i_coeff_inc < scat_data_spectral_raw[i_ss][i_se]
                                   .backward_peak_data_imag.nrows();
                 i_coeff_inc++) {
              for (Index i = 0; i < scat_data_spectral_raw[i_ss][i_se]
                                        .backward_peak_data_imag.ncols();
                   i++) {
                interp(
                    scat_data_spectral[i_ss][i_se].backward_peak_data_imag(
                        joker, t_index, i_coeff_inc, i),
                    itw,
                    scat_data_spectral_raw[i_ss][i_se].backward_peak_data_imag(
                        joker, t_index, i_coeff_inc, i),
                    freq_gp);
              }
            }
        ...
        // been confirmed to be the same), hence only need to copy over/reassign
        // the data.
        scat_data_spectral[i_ss][i_se].pha_mat_data_real =
            scat_data_spectral_raw[i_ss][i_se].pha_mat_data_real;
        scat_data_spectral[i_ss][i_se].ext_mat_data_real =
            scat_data_spectral_raw[i_ss][i_se].ext_mat_data_real;
        scat_data_spectral[i_ss][i_se].abs_vec_data_real =
            scat_data_spectral_raw[i_ss][i_se].abs_vec_data_real;
        scat_data_spectral[i_ss][i_se].forward_peak_data_real =
            scat_data_spectral_raw[i_ss][i_se].forward_peak_data_real;
        scat_data_spectral[i_ss][i_se].backward_peak_data_real =
            scat_data_spectral_raw[i_ss][i_se].backward_peak_data_real;
        scat_data_spectral[i_ss][i_se].pha_mat_data_imag =
            scat_data_spectral_raw[i_ss][i_se].pha_mat_data_imag;
        scat_data_spectral[i_ss][i_se].ext_mat_data_imag =
            scat_data_spectral_raw[i_ss][i_se].ext_mat_data_imag;
        scat_data_spectral[i_ss][i_se].abs_vec_data_imag =
            scat_data_spectral_raw[i_ss][i_se].abs_vec_data_imag;
        scat_data_spectral[i_ss][i_se].forward_peak_data_imag =
            scat_data_spectral_raw[i_ss][i_se].forward_peak_data_imag;
        scat_data_spectral[i_ss][i_se].backward_peak_data_imag =
