...
void xml_read_from_stream(istream& is_xml,
  ...
  if (sssdata.pha_mat_data_real.nshelves() != sssdata.f_grid.nelem()) {
    throw runtime_error(
        "Number of frequencies in f_grid and pha_mat_data_real "
        "not matching!!!");
  }
  if (sssdata.pha_mat_data_imag.nshelves() != 1 &&
      sssdata.pha_mat_data_imag.nshelves() != sssdata.f_grid.nelem()) {
    throw runtime_error(
        "Number of frequencies in f_grid and pha_mat_data_imag "
        "not matching!!!");
  }
  ...
  xml_read_from_stream(
      is_xml, sssdata.forward_peak_data_real, pbifs, verbosity);
  xml_read_from_stream(
      is_xml, sssdata.forward_peak_data_imag, pbifs, verbosity);
  xml_read_from_stream(
      is_xml, sssdata.backward_peak_data_real, pbifs, verbosity);
  xml_read_from_stream(
      is_xml, sssdata.backward_peak_data_imag, pbifs, verbosity);
...
void xml_write_to_stream(ostream& os_xml,
  ...
  os_xml << '\n';
  xml_write_to_stream(
      os_xml, PTypeToString(sssdata.ptype), pbofs, "", verbosity);
  ...
  xml_write_to_stream(
      os_xml, sssdata.forward_peak_data_real, pbofs, "", verbosity);
  xml_write_to_stream(
      os_xml, sssdata.forward_peak_data_imag, pbofs, "", verbosity);
  xml_write_to_stream(
      os_xml, sssdata.backward_peak_data_real, pbofs, "", verbosity);
  xml_write_to_stream(
      os_xml, sssdata.backward_peak_data_imag, pbofs, "", verbosity);
