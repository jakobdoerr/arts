    ...
    for( Index i_se=0; i_se<N_se; i_se++ )
    {
      ...
      if( nf_se != 1 )
      {
        ...
        else
        {
          for( Index f=0; f<nf_se; f++ )
          {
            if( !is_same_within_epsilon( scat_data_spectral[i_ss][i_se].f_grid[f],
                                         f_grid[f], 0.5e-9 ) )
            {
              ostringstream os;
      ...
      else
      {
        if( (abs(1.-scat_data_spectral[i_ss][i_se].f_grid[0]/f_grid[0])
             > dfrel_threshold) or
            (abs(1.-scat_data_spectral[i_ss][i_se].f_grid[0]/f_grid[nf-1])
             > dfrel_threshold) )
        {
          ostringstream os;
          os << "Frequency entry (f=" << scat_data_spectral[i_ss][i_se].f_grid[0]
             << "Hz) of scattering element #" << i_se << "\n"
             << "in scattering species #" << i_ss
             << " is too far (>" << dfrel_threshold*1e2
             << "%) from one or more\n" << "of the f_grid limits (fmin="
      ...
      // doesn't allow this. see FIXME at start.).
      {
        ...
        bs2 << " must be ssd.f_grid.nelem() (=" << nf_se << "),\n"
            << "but scattering element #" << i_se
            << " in scattering species #" << i_ss << " is ";
        Index nf_sd = scat_data_spectral[i_ss][i_se].pha_mat_data_real.nshelves();
        if( nf_sd != nf_se ) //&& nf_sd != 1)
        {
          ostringstream os;
        ...
        {
          ostringstream os;
          os << bs1.str() << "forward_peak_data_imag" << bs2complx.str() << nf_sd << ".";
          throw runtime_error( os.str() );
        }
        ...
        {
          ostringstream os;
          os << bs1.str() << "backward_peak_data_imag" << bs2complx.str() << nf_sd << ".";
          throw runtime_error( os.str() );
        }
      }
